...
Mongo.prototype.subscribe = function( name, consumer, fn ){
  ...
  // this._subscriptions[ name ] = this._channel.subscribe({ name: name }, function( doc ){
  ...
  if( consumer || fn ){
    ...
    this.on( 'subscribe', function subscribe( channel ){
      if( name === channel ){
        function message( channel, msg ){
          if( name === channel ){
            ...
            this._subscriptions[ name ] = this._channel.subscribe({ name: name }, function( doc ){
              ...
              if( self._nodeId != doc.nodeId )
                consumer.apply( null, parse( doc.args ) );
            });
          }
        };
        ...
        self.on( 'unsubscribe', function unsubscribe( channel ){
          if( name === channel){
        ...
        self.removeListener( 'subscribe', subscribe );
